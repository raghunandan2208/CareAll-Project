...
# Care taking table
mycursor.execute("CREATE TABLE IF NOT EXISTS caretaking (PK_request_id INT AUTO_INCREMENT PRIMARY KEY, FK_elder_id integer references elders(PK_elder_id), FK_younger_id integer references youngers(PK_younger_id))")

# Younger account balance
mycursor.execute("CREATE TABLE IF NOT EXISTS younger_money (PK_request_id INT AUTO_INCREMENT PRIMARY KEY, FK_younger_id integer references youngers(PK_younger_id), balance Integer)")
